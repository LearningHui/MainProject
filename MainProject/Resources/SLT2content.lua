...
-- @return string
function ert_slt_ad_2015.precompile(template, start_tag, end_tag)
...
-- @return { string }
function ert_slt_ad_2015.get_dependency(template, start_tag, end_tag)
...
function ert_slt_ad_2015.loadstring(template, start_tag, end_tag, tmpl_name)
	...
	template = ert_slt_ad_2015.precompile(template, start_tag, end_tag)
	...
	local ret = { name = tmpl_name or '=(ert_slt_ad_2015.loadstring)' }
...
-- @return { name = string, code = string / function }
function ert_slt_ad_2015.loadfile(filename, start_tag, end_tag)
    ...
    return ert_slt_ad_2015.loadstring(t)
	--local fin = assert(io.open(filename))
	--local all = fin:read('*a')
	--fin:close()
	--return ert_slt_ad_2015.loadstring(all, start_tag, end_tag, filename)
end
...
function ert_slt_ad_2015.render_co(t, env)
	local f
	if setfenv == nil then -- lua 5.2
		if env ~= nil then
...
-- @return string
function ert_slt_ad_2015.render(t, env)
	local result = {}
	local co = coroutine.create(ert_slt_ad_2015.render_co(t, env))
...
function ert_slt_ad_2015.renderfile(filename,paramsTable)
    local file = ert_slt_ad_2015.loadfile(filename);
    local render_file = ert_slt_ad_2015.render(file, paramsTable);
    return render_file;
end

-- 将loadstring和render函数封装,供客户端调用
ert_slt_ad_2015["load_render_string"] =
function (content)
    local page = ert_slt_ad_2015.loadstring(content);
    page = ert_slt_ad_2015.render(page,nil);
    return page;
end
